...
impl StateImpl of StateTrait {
    ...
    fn process(
      ...
      let State{color, position, quit, } = self;
      match message {
          Message::Quit => self = State { color: color, position: position, quit: true,  },
          Message::Echo(s) => s.print(),
          Message::Move(p) => self = State { color: color, position: p, quit: quit,  },
          Message::ChangeColor(new_color) => self = State { color: new_color, position: position, quit: quit,  },
      }
    }
}
